...
func (p *Parameter) UnmarshalUnstructured(data interface{}) error {
	if err := FromUnstructured(data, &p.CommonValidations); err != nil {
		return err
	}
	if err := FromUnstructured(data, &p.Refable); err != nil {
		return err
	}
	if err := FromUnstructured(data, &p.SimpleSchema); err != nil {
		return err
	}
	if err := FromUnstructured(data, &p.VendorExtensible); err != nil {
		return err
	}
	return FromUnstructured(data, &p.ParamProps)
}
